...
from model_laddersider import LadderSide_GNN, GNN_graphpred_ladderside
...
def main():
    ...
    #set up model
    model = GNN_graphpred_ladderside(args.num_layer, args.emb_dim, num_tasks, JK = args.JK, drop_ratio = args.dropout_ratio, graph_pooling = args.graph_pooling)
    model.to(device)
    ...
    # for i in model.named_parameters():
    #     print(i)
    with open('outputs/ladderside_result.log', 'a+') as f:
        f.write(args.input_model_file + args.dataset + ' ' + str(args.runseed) + ' ' + str(max(np.array(test_acc_list))))
        f.write('\n')
